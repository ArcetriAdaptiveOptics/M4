...
from argos.util.executor import MultiThreadExecutor, FunctionTask, TaskTimeoutError
...
class Snapshooter(object):
    ...
    def _createSnapshotDir(self):
        ...
        self._logger.notice("Creating snapshot directory %s" % (self._snapshotDir))
    ...
    def _snapshotLGSWController(self, lgswOverview):
        lgswOverview.update(self._getLgswCtrlClient().getSnapshot(SnapshotPrefix.LGSW))

    def _snapshotLASController(self, lasOverview):
        lasOverview.update(self._lasCtrlClient().getSnapshot(SnapshotPrefix.LAS))

    def _snapshotLANController(self, lanOverview):
        lanOverview.update(self._lanCtrlClient().getSnapshot(SnapshotPrefix.LAN))

    def _snapshotTIPTILTController(self, tiptiltOverview):
        tiptiltOverview.update(
            self._tiptiltCtrlClient().getSnapshot(SnapshotPrefix.TIPTILT)
        )

    def _snapshotAArbController(self, aarbOverview):
        aarbOverview.update(self._aarbClient().getSnapshot(SnapshotPrefix.AARB))
    ...
    def _buildOverviewHeaderAndDumpIt(self):
        ...
        pyfits.writeto(
            self._snapshotPath(self.FILE_OVERVIEW), np.array([1, 2]), self._overviewHdr
        )
    ...
    def _generateAarbAoLoopSnapshot(self):
        ...
        pyfits.writeto(
            self._snapshotPath(self.FILE_AO_GAIN), gainVector, self._getOverviewHeader()
        )
    ...
    def _takeFieldCameraSnapshot(self):
        images = (
            self._getLasDevicePool()
            .getFieldCamera()
            .getFrames(self._currentConfig.getNumOfFieldCameraFrames())
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_FIELD_CAM_FRAMES),
            images,
            self._getOverviewHeader(),
        )

    def _createTaskFor(self, f, *args, **kwds):
        self._futureQueue.put(self._executor.execute(FunctionTask(f, *args, **kwds)))

    def _takePupilCameraSnapshot(self):
        images = (
            self._getLasDevicePool()
            .getPupilCamera()
            .getFrames(self._currentConfig.getNumOfPupilCameraFrames())
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_PUPIL_CAM_FRAMES),
            images,
            self._getOverviewHeader(),
        )
    ...
    def _takeOsciSnapshot(self):
        ...
        pyfits.writeto(
            self._snapshotPath(self.FILE_OSCILLOSCOPE),
            curves,
            self._getOverviewHeader(),
        )
    ...
    def _takeFrameBufferSnapshot(self, config):
        ...
        pyfits.writeto(
            self._snapshotPath(self.FILE_SLOPES), loopdata[FB.KEY_DIAG_ALL_SLOPES], hdr
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_SLOPE_FRAME_COUNTERS),
            loopdata[FB.KEY_DIAG_SLOPE_FRAME_COUNTERS],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_HVC_TT_JITTER),
            loopdata[FB.KEY_DIAG_HVC_TT_JITTER],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_PIEZO_COMMANDS),
            loopdata[FB.KEY_DIAG_HVC_COMMANDS],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_PIEZO_POSITIONS),
            loopdata[FB.KEY_DIAG_HVC_POSITIONS],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_APD_COUNTERS),
            loopdata[FB.KEY_DIAG_APD_COUNTERS],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_ASM_INT_MODES),
            loopdata[FB.KEY_DIAG_ASM_INT_MODES],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_FFCOMMANDS),
            loopdata[FB.KEY_DIAG_ASM_FF_COMMANDS],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_ASM_DIST_AVGS),
            loopdata[FB.KEY_DIAG_ASM_DIST_AVGS],
            hdr,
        )

        pyfits.writeto(
            self._snapshotPath(self.FILE_LGSW_CCD_FRAMES),
            loopdata[FB.KEY_DIAG_CCD_RAW_FRAMES],
            hdr,
        )
        pyfits.writeto(
            self._snapshotPath(self.FILE_LGSW_CCD_FRAME_COUNTERS),
            loopdata[FB.KEY_DIAG_CCD_FRAME_COUNTERS],
            hdr,
        )
    ...
    def _takeCurrentSlopesOffsetVectorSnapshot(self, config):
        ...
        pyfits.writeto(
            self._snapshotPath(self.FILE_SLOPES_OFFSET),
            slopesOffset.toNumPyArray(),
            hdr,
        )
    ...
    def _takePatrolCameraSnapshot(self, config):
        ...
        def takeFramesFromBlueCamera():
            ...
            pyfits.writeto(
                self._snapshotPath(self.FILE_PAT_CAM_BLUE),
        ...
        def takeFramesFromRedCamera():
            ...
            pyfits.writeto(
                self._snapshotPath(self.FILE_PAT_CAM_RED),
        ...
        def takeFramesFromYellowCamera():
            ...
            pyfits.writeto(
                self._snapshotPath(self.FILE_PAT_CAM_YELLOW),
    ...
    def _generateLGSWSnapshot(self):
        self._futureQueue.put(
            self._executor.execute(
                FunctionTask(self._takeFrameBufferSnapshot, self._currentConfig)
            )
        )
        ...
        self._futureQueue.put(
            self._executor.execute(
                FunctionTask(
                    self._takeCurrentSlopesOffsetVectorSnapshot, self._currentConfig
                )
            )
        )
        ...
        self._futureQueue.put(
            self._executor.execute(
                FunctionTask(self._takePatrolCameraSnapshot, self._currentConfig)
            )
        )
