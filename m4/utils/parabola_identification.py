...
class ParabolaActivities:
    ...
    def par_mask_on_ott(self, image):
        ...
        """
        image_masked_central_fid = geo.draw_mask(
            image,
            int(image.shape[0] / 2),
            int(image.shape[1] / 2),
    ...
    def fiduciali(self, image):
        ...
        sel_image = (image_df["area"] < 1000) & (
            image_df["area"] > np.median(image_df["area"]) / 2
        )
        imaf = np.array(
    ...
    def check_concentricity(self, image):
        ...
        """
        min_inner_markers_rejection_radius = np.array(
            ["n_pixel1", "n_pixel2", "n_pixel3"]
        )
        max_inner_markers_rejection_radius = np.array(
            ["n_pixel1", "n_pixel2", "n_pixel3"]
        )
        for i in range(min_inner_markers_rejection_radius.shape[0]):
            mask1 = self._draw_mask(
                image,
                int(image.shape[0] / 2),
                int(image.shape[1] / 2),
                min_inner_markers_rejection_radius[i],
            )
            mask2 = self._draw_mask(
                image,
                int(image.shape[0] / 2),
                int(image.shape[1] / 2),
