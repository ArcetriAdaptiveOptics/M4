...
def crop_frame(imgin):
    ...
    img = imgin.data[
        cir[0] - cir[2] : cir[0] + cir[2] + 1, cir[1] - cir[2] : cir[1] + cir[2] + 1
    ]
    m = imgin.mask[
        cir[0] - cir[2] : cir[0] + cir[2] + 1, cir[1] - cir[2] : cir[1] + cir[2] + 1
    ]
...
def ott_calib(
    tnott, tnpar, zlist=[1, 2, 3, 4, 7, 8], filtfreq=None, px_ott=0.00076, crpar=None
):
    """
    ...
    if filtfreq is not None:
        par = th.comp_filtered_image(
            par, d=0.00076, verbose=True, disp=False, freq2filter=filtfreq
        )
...
def mask_edge(img, pix2remove):
    cir = geo.qpupil(-1 * img.mask + 1)
    center_mask = geo.draw_mask(
        np.ones(img.shape), cir[0], cir[1], cir[2] - pix2remove, out=1
    )
    master_mask = (-1 * img.mask + 1) * (-1 * center_mask + 1)
    master_mask = -1 * master_mask + 1
    img = np.ma.masked_array(img.data, master_mask)
...
def compSlopXY2(img, px, rfact, thr=None):
    ...
    slix = np.ma.masked_array(
        (imgr - np.roll(imgr, (1, 0), axis=(0, 1))) / (px * rfact), slm < 2
    )
    sliy = np.ma.masked_array(
        (imgr - np.roll(imgr, (0, 1), axis=(0, 1))) / (px * rfact), slm < 2
    )
    sli = np.ma.sqrt(slix**2 + sliy**2)
    if thr is not None:
