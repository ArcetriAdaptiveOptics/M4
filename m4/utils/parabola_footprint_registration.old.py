...
class ParabolaFootprintRegistration:
    ...
    def marker_finder(self, cgh_image, ott_image):
        ...
        sel_cgh = (cghf_df["area"] < 1000) & (
            cghf_df["area"] > np.median(cghf_df["area"]) / 2
        )
        sel_ott = (ottf_df["area"] < 1000) & (
            ottf_df["area"] > np.median(ottf_df["area"]) / 2
        )
        ...
        fit2d = interp2d(
            np.arange(npix),
            np.arange(npix),
            np.array(cgh_image != 0, dtype="float"),
            kind="linear",
        )
    ...
    def cgh_coord_tranform_and_trig_interpolatio(
        self, cgh_image, ott_image, cgh_not_blobs, polycoeff
    ):
    ...
    def plot_difference_and_marker_view(self, difference):
        ...
        title(
            ...
            % (npix2crop, np.std(difference_crop[cropmask]))
        )
        colorbar()

        nrow = int32(np.ceil(self._nmarkers / 3))
