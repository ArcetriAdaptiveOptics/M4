...
import numpy as np
import h5py
from opticalib.analyzer import mode_rebinner as rebinner
...
class AccelerometersData:
    ...
    def __init__(self):
        """The constructor"""
        self._rebinnig_factor = (
            OpcUaParameters.accelerometers_dt / OpcUaParameters.accelerometers_dt_plc
        )
        self.tt = None
    ...
    def convertAndSaveData(self, start, final_destination):
        ...
        if self._rebinnig_factor != 1:
            rebinning_size = int(vec.shape[0] / self._rebinnig_factor)
            v_list = []
    ...
    def counts_to_ms2(self, vec):
        ...
        for i in range(vec.shape[1]):
            cal_vec = (
                (vec[:, i] / OpcUaParameters.accelerometers_plc_totcounts)
                * plcfs
                * 9.81
                / sens
            )
