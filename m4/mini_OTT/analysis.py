...
class Analysis():
    ...
    def longTerm_analysis(self, zc1, zc2=None, ntick=6, figure=True):
        ...
        plt.plot(timeh[start:stop], zern[start:stop,zc1]*10**9, 'k',
                 label='Z' + str(zc1))
        if zc2 is not None:
            plt.plot(timeh[start:stop], zern[start:stop,zc2]*10**9, 'r',
                     label='Z' + str(zc2))
    #    plt.plot(timeh[start:stop],zern[start:stop,3]*10**9,'k',label='Z2')
    #    plt.plot(timeh[start:stop],zern[start:stop,2]*10**9,'r',label='Z3')
        if figure is True:
            plt.xticks(timeh,taxis[::len(taxis)/ntick],
                       rotation=0)#,fontsize=fsize)
