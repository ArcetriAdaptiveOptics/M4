...
class CapSensCalibSimulator:
    ...
    conf = '---/MyConf.yaml'
    ott, interf, dm = start.create_ott(conf)
    csc = CapSensCalibSimulator(ott, interf, dm)
    """
    ...
    def main_test(self):
        ...
        self.move2petalo(6, 0)

        ampiezza = 10e-9
        rel = False
        self._dm.setRandomCommandToSegActs(ampiezza, rel)
        self.immagine(1, True, ind)

        inc = 100e-9
        rel = True
        self._dm.setIncreaseToSegActs(inc, rel)
        self.immagine(2, True, ind)

        rel = False
        # dm.act_zero(rel)
        self._dm.setZerosToSegActs()
        self.immagine(3, True, ind)

    def test_noise(self):
        data_file_path = (
            fold_name.SIMUL_DATA_CALIB_DM_FOLDER
            + "\\PassettiWithNoise\\20220801_110511"
        )
        numbers_array = np.array([2, 3, 5, 7])
        tidy_or_shuffle = 0
        noise.noise_vibrations(data_file_path, numbers_array, tidy_or_shuffle)

    def main_passetti_and_Unwrap(self, Nstep, Nact):
        """
        ...
        """
        D = 0
        cartella_imm = self.passetti_con_Rumore(Nstep, D, freq=25, ind=True)
        cartella_act = "PositionActuators\\20220519_130221"

        Z, mask, mask2 = self.passetti_postporocess(cartella_imm, cartella_act, Nact)
        ...
        U = self.unwrapp(Z, 632.8e-9, 150e-9)
        ax2.plot(x, U, "-x")
        ax2.plot(x, Z, "-x")
        plt.xlabel("step")
        ...
        mat = mat.transpose()
        np.savetxt(
            cartella_imm + "/position_Actuator" + str(Nact) + ".txt",
            mat,
            header="original    unwrap",
        )

    def main_Unwrap(self, Nact):
        """
        Faccio l'unwrap delle immagini generate con
        "passetti" o "passettiwithnoise"
    ...
    def passetti(self, N=1, D=20e-9, ind=True):
        """
        ...
        self._dm.setIncreaseToSegActs(inc, rel)
        ima = self._interf.acquire_phasemap()
        ...
        self._dm.setIncreaseToSegActs(inc, rel)
        ima = self._interf.acquire_phasemap()
        self._interf.save_phasemap(dir, "0000.fits", ima)
        ...
        for x in range(0, N):
            ...
            imname = "{xx+1:04}" + ".fits"
            self._interf.save_phasemap(dir, imname, ima)
    ...
    def passetti_con_Rumore(self, N=1, D=20e-9, freq=25, ind=True):
        """
        ...
        self._dm.setIncreaseToSegActs(inc, rel)
        ima = self._interf.acquire_phasemap()
        self._interf.save_phasemap(dir, "img_0000.fits", ima)
        ...
        Ax = 2.1e-8 * 1e3
        Frx = 14
        start_x = 2 * np.pi * np.random.rand()
        # A in mm, Fr in Hz
        Ay = 2.2e-8 * 1e3
        Fry = 62
        start_y = 2 * np.pi * np.random.rand()
        # A in mm, Fr in Hz
        Az = 2.1e-8 * 1e3
        Frz = 158
        start_z = 2 * np.pi * np.random.rand()
        # A in mm, Fr in Hz
        Atx = 2.2e-8 * 1e3
        Frtx = 10
        start_tx = 2 * np.pi * np.random.rand()
        # A in mm, Fr in Hz
        Aty = 2.4e-8 * 1e3
        Frty = 90
        start_ty = 2 * np.pi * np.random.rand()
        # A in mm, Fr in Hz
        ...
        f.write(
            "\nAmplitues noise (mm,mrad) (x,y,z,tx,ty):    "
            + str(Ax)
            + "    "
            + str(Ay)
            + "    "
        ...
        f.write(
            "\nFrequencies noise (Hz) (x,y,z,tx,ty):    "
            + str(Frx)
            + "    "
            + str(Fry)
            + "    "
        ...
        for xx in range(0, N):
            ...
            self._ott.parabola.setPosition(np.array([x, y, z, tx, ty, 0]))
    ...
    def passetti_postporocess(self, cartella_imm, cartella_act, N):
        """
    ...
    def immagine(self, numFig=0, plotta=True, indet=True):
        """
        plot immagine
        """
    ...
    def unwrapp(self, x, phase, threshold):
        """
        ...
        for ii in range(1, n):
            if np.absolute(dx[ii]) < threshold:
                ...
                x_un[ii] = x_un[ii - 1] - np.sign(dx[ii]) * (
                    phase - np.absolute(dx[ii])
                )
