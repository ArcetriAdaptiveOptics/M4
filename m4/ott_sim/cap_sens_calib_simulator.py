...
class CapSensCalibSimulator():
    ...
    def main_passetti_and_Unwrap(self, Nstep, Nact):
        ...
        mat = np.array([Z, U])
        mat = mat.transpose()
        np.savetxt(cartella_imm+'/position_Actuator'+str(Nact)+'.txt',
                   mat, header='original    unwrap')
    ...
    def main_Unwrap(self, Nact):
        ...
        Z, mask, mask2 = self.passetti_postporocess(cartella_imm,
                                               cartella_act, Nact)
    ...
    def passetti_con_Rumore(self, N=1, D=20e-9, freq=25, ind=True):
        ...
        plt.xlabel('time(s)')
        plt.ylabel('noise')
        plt.legend((l1, l2, l3, l4, l5),('x(m)', 'y(m)', 'z(m)',
                                         'tx(rad)', 'ty(rad)'))
        ...
        f.write("\nAmplitues noise (mm,mrad) (x,y,z,tx,ty):    "+str(Ax)+
                "    "+str(Ay)+"    "+str(Az)+"    "+str(Atx)+"    "+str(Aty))
        f.write("\nFrequencies noise (Hz) (x,y,z,tx,ty):    "+
                str(Frx)+"    "+str(Fry)+"    "+str(Frz)+"    "+str(Frtx)+
                "    "+str(Frty))
        f.close()
