...
def analysis_req(data_file_path, zernike_vector_to_subtract, step=None, offset=None):
    ...
    image50 = req_check.robustImageFromDataSet(
        50, data_file_path, zernike_vector_to_subtract, offset
    )
    print("Creating cube 100")
    image100 = req_check.robustImageFromDataSet(
        100, data_file_path, zernike_vector_to_subtract, offset
    )
    print("Creating cube 300")
    image300 = req_check.robustImageFromDataSet(
        300, data_file_path, zernike_vector_to_subtract, offset
    )
    ...
    slop_list, diff_piston_list, roc_list, rms31, rms500 = fromImagesToReq(
        image_list, None, step
    )
    ...
    plotAndSaveForReqAnalysis(
        x, y, "sqrt(n_frames)", "rms_slope [arcsec]", dove, "slope.png"
    )
    # GRAFICO DIFF PISTON
    y = np.array(diff_piston_list)
    plotAndSaveForReqAnalysis(
        x, y, "sqrt(n_frames)", "diff_piston [m]", dove, "diff_piston.png"
    )
    # GRAFICO ROC
    y = np.array(roc_list)
    ...
    y = np.array(rms31)
    plotAndSaveForReqAnalysis(
        x, y, "sqrt(n_frames)", "rms_31mm [m]", dove, "rms_31mm.png"
    )
    # GRAFICO RMS 500 MM
    y = np.array(rms500)
    plotAndSaveForReqAnalysis(
        x, y, "sqrt(n_frames)", "rms_500mm [m]", dove, "rms_500mm.png"
    )
