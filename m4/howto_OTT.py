...
tnc = main.calibrate_PARAndRM(
    ott, interf, command_amp_vector, nPushPull, n_frames, delay=0
)
...
dofidf = np.array([0])  # focus, parpist
# alignment of tilt onlu
zern2corrf = np.array([0, 1])
dofidf = np.array([3, 4])  # TipTilt, RmTip, RmTilt
# alignment of tilt and coma
zern2corrf = np.array([0, 1, 3, 4])
dofidf = np.array([1, 2, 3, 4])  # TipTilt, Coma,ParTip, ParTilt, RmTip, RmTilt
...
tna = main.align_PARAndRM(
    ott, interf, tnc, zern2corrf, dofidf, n_frames=1, doit=True, tnPar=tnPar
)
