...
def _createZabbixAPI():
    zapi = ZabbixAPI(
        url="https://192.168.22.22/zabbix/", user="Admin", password="zabbix"
    )
    return zapi
...
def _getZabbixMetrics(zapi, host, key):
    ...
    # Get specified host
    print("----------------------------")
    thehost = zapi.do_request(
        "host.get",
        {"filter": {"host": host}, "selectItems": "extend", "output": "extend"},
    )
    ...
    if itemId is None:
        print(
            "HALTING. There was no item defined on host {} with name: {}".format(
                host, key
            )
        )
        sys.exit(2)
    print(
        "Found item {} on host {} with item id/type {}/{}".format(
            key, host, itemId, itemType
        )
    )

    # https://www.zabbix.com/documentation/2.2/manual/api/reference/history/get
    print("----------------------------")
    history = zapi.do_request(
        "history.get",
        {
            "history": itemType,
    ...
    avehist = reduce(
        (lambda x, y: float(x) + float(y)), [d["value"] for d in (history["result"])]
    ) / len(history["result"])
