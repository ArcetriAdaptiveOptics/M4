...
class ZernikeGenerator(object):
    ...
    def _makeRectGrid(self, nPixel):
        Y, X= np.mgrid[-1+ 1. / nPixel: 1- 1. / nPixel: nPixel * 1j,
                       -1+ 1. / nPixel: 1- 1. / nPixel: nPixel * 1j]
        return X, Y


    def _rectToPolar(self, X, Y):
        r = np.sqrt(X**2 + Y**2)
        th= np.arctan2(Y, X)
        th= np.where(th< 0, 2*np.pi + th, th)
        return r, th
    ...
    def transform(self, xp, yp, xc, yc, alpha):
        return (xp-xc)/alpha, (yp-yc)/alpha


    def getZernikeDictWithOffset(self, indexVector, xc=0, yc=0, alpha=1):
        ret= {}
        for index in indexVector:
            ret[index]= self.getZernikeWithOffset(index, xc, yc, alpha)
        return ret


    def getZernikeWithOffset(self, index, xc=0, yc=0, alpha=1):
        xAct, yAct= self.getXYCoordinates()   
        #xc=.5/12; yc=0.; alpha=12./8
        xHatAct, yHatAct= self.transform(xAct, yAct, xc, yc, alpha)
        return self.getZernikeAtCoordinates(index, xHatAct, yHatAct)
    ...
    def getZernikeAtCoordinates(self, index, xCoordArray, yCoordArray):
        rhoArray, thetaArray= self._rectToPolar(xCoordArray, yCoordArray)
        return self._polar(index, rhoArray, thetaArray)
