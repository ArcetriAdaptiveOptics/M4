...
class OttParameters:
    ...
    parab_max_displacement = np.array(
        [0, 0, 3, 10, 10, 0]
    )  # range of maximum allowed displacement
    rm_max_displacement = np.array(
        [0, 0, 0, 10, 10, 0]
    )  # range of maximum allowed displacement
    m4_max_displacement = np.array(
        [0, 0, 0, 1, 1, 0]
    )  # range of maximum allowed displacement
    ...
    PAR_SLIDER_KIN_OFFSET = 0.768  # 1.29157
    PAR_SLIDER_DIRECTION = -1
    RM_SLIDER_KIN_OFFSET = 0.608
    RM_SLIDER_DIRECTION = 1
    ANGLE_OFFSET = 0.0 # deg
    # -------------------- #
    ...
    M4_MECHANICAL_PUPIL_XYRADIUS = np.array(
        [458, 458, 458]
...
class OpcUaParameters:
    ...
    accelerometers_plc_id = np.array(
        [5, 6, 7, 8]
    )  # final vector is time, accelerom, --> first accelerom is 1
    accelerometrs_directions = ["", "", "", "", "X", "Z", "Y", "Z"]
    accelerometers_sensitivity = np.array(
        [0.0, 0, 0, 0, 0.1, 0.1, 0.1, 10]
    )  # accel sentivity [V/g]
    accelerometers_plc_range = np.array(
        [0.0, 0, 0, 0, 0.32, 0.32, 0.32, 1.28]
    )  # this is the full range of measurement. i.e. the total span, where the signal is sampled with 2**24 counts
    accelerometers_plc_totcounts = (
        2**24
