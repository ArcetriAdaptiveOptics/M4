...
a5=z[:,5]
a6 = z[:,6]
plot(th.runningMean(a5,10),'.'); title(tn+' Ast-Runnmean')



q0=th.averageFrames(100,800,fl)
i=0
imshow(th.removeZernike(th.frame(i, fl)-q0,[1,2,3,4]))
rr = []
for i in range(nf):
    qi = th.removeZernike(th.frame(i,fl)-q0,[1,2,3,4,7,8])
    rr.append(qi.std())

plot(th.runningMean(rr,10), '.'); title(tn+' RMS-ref')

zz = th.zernikePlot(fl)
nn = 100
dd = th.runningDiff(tn,2)
plot(tvec[4::2], dd, '.');title(tn+': RunningDiff-RMS,2min'); xlabel('Time since start [hr]')
q=[]
for i in range(10):
    qq=th.removeZernike(th.averageFrames(i*100,i*100+nn-1,fl))
    q.append(qq)

clf();imshow(q[1]-q[0]);colorbar();title(tn+', avg diff I'); std(q[1]-q[0])
clf();imshow(q[4]-q[3]);colorbar();title(tn+', avg diff II');std(q[4]-q[3])
clf();imshow(q[8]-q[7]);colorbar();title(tn+', avg diff III');std(q[8]-q[7])
...
stimg = std(qa,0)
clf();imshow(stimg, vmax=50e-9);colorbar();title(tn+' PixStddev')
