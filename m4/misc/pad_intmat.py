...
#    base = '/mnt/data/M4/Data/M4Data/OPTData/PAR_Meas/20220620/'
    ...
    clf(); imshow(img2); colorbar(); title(fname)
    foc = ccx[3]
    ast = ccx[4:6]
    tref=ccx[8:10]
    cc = array((foc, ast, tref))
    return cc
...
cmd0 = matmul(invmat, pz0m)
pz0m - matmul(cmat, cmd0)
cmd1 = array((cmd0, cmd0))

rmat = rotmat(120)
cmat1 = matmul(cmat, rmat.T)

ccm = concatenate((cmat, mat1), 1)

invmat2 = np.linalg.pinv(ccm)
cmd2 = matmul(invmat2, pz0m)
